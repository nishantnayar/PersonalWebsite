...
function SidebarContent({ onNavClick }: { onNavClick?: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full py-8 px-6">
      ...
      <div className="mb-8">
        <div className="relative w-16 h-16 rounded-full overflow-hidden bg-gradient-to-br from-indigo-100 to-violet-100 mb-4 ring-2 ring-white shadow-sm">
          {avatarUrl ? (
            ...
            <span className="absolute inset-0 flex items-center justify-center text-2xl font-bold text-indigo-300 select-none">
              {name.charAt(0)}
            </span>
          )}
        </div>
        <p className="font-semibold text-gray-900 leading-snug">{name}</p>
        <p className="text-sm text-gray-500 mt-0.5 leading-snug">{title}</p>
        {location && (
        ...
        <div className="space-y-0.5">
          ...
          {socialLinks.map(({ href, icon, label }) => (
            <a
              ...
              className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-gray-500 hover:text-accent hover:bg-gray-50 transition-colors"
            >
              {icon}
              {label}
            </a>
      ...
      {/* Copyright */}
      <p className="mt-auto pt-8 text-xs text-gray-300 px-3">
        &copy; {year} {name}
      </p>
    </div>
  );
    ...
    <>
      ...
      {mobileOpen && (
        <>
          ...
          />
          <aside className="fixed left-0 top-0 h-full w-64 bg-white z-50 lg:hidden border-r border-gray-100 overflow-y-auto">
            <button
              onClick={() => setMobileOpen(false)}
